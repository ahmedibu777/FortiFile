...
/**
 * Optional Swing-based graphical user interface.
 */
public class SwingGUI {
    ...
    private void runCrypto(boolean encrypt, JProgressBar bar, JLabel status, JFrame parent) {
        JFileChooser chooser = new JFileChooser();
        int result = chooser.showOpenDialog(parent);
        if (result != JFileChooser.APPROVE_OPTION) return;

        File input = chooser.getSelectedFile();
        String defaultOutput = input.getAbsolutePath() + (encrypt ? ".enc" : ".dec");
        String outputPath = JOptionPane.showInputDialog(parent, "Save output as:", defaultOutput);
